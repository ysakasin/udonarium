...
class RuinBreakers < DiceBot
  ...
  def check_roll(command)
    ...
    critical_border = m[4]&.to_i || [success_rate.div(5), 1].max
  ...
  def roll_fp_damage(command)
    ...
    roll_result, dice_str, = roll(1 + ruin_point_tens, 10)

    sequence = [
      "(1+#{ruin_point_tens})D10+#{ruin_point_ones}",
      "#{roll_result}[#{dice_str}]+#{ruin_point_ones}",
      "#{roll_result + ruin_point_ones}ダメージ"
    ]
  ...
  def roll_fp_recovery(command)
    ...
    roll_result, dice_str, = roll(dice_count, 10)
